...
namespace RanchingSensors
{
	public class CrittersSensorConfig : IBuildingConfig
	{
		...
		{
			string id = CrittersSensorConfig.ID;
			int width = 1;
			int height = 1;
			...
			GeneratedBuildings.RegisterWithOverlay(OverlayModes.Logic.HighlightItemIDs, CrittersSensorConfig.ID);
		...
		public override void DoPostConfigureComplete(GameObject go)
		{
			...
			GeneratedBuildings.RegisterLogicPorts(go, LogicSwitchConfig.OUTPUT_PORT);
			CrittersSensor critterSensor = go.AddOrGet<CrittersSensor>();
